      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-200 bg-white">
        <table className="w-full min-w-[720px] text-sm">
          <thead className="bg-neutral-100 text-left text-neutral-600">
            <tr>
              <th className="px-3 py-2 font-medium">Receipt #</th><th className="px-3 py-2 font-medium">Account no.</th><th className="px-3 py-2 font-medium">Customer</th>
          ...
          <tbody className="divide-y divide-neutral-100">
            {receipts.length === 0 && <tr><td colSpan={7} className="px-3 py-6 text-center text-neutral-500">{lp.q ? "No receipts match your search." : "No receipts yet."}</td></tr>}
            {receipts.map((r) => (
              <RowLink key={r.id} href={`/receipts/${r.id}`} className={r.reversed_at ? "opacity-60" : ""}>
                <td className="px-3 py-2">
                  <span className="font-mono text-xs font-medium text-brand">{r.receipt_number ?? "—"}</span>
